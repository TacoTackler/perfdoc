...
class TextureTest : public VulkanTestHelper
{
public:
	//test with uncompressed texture format
	bool testUncompressedFormat(bool positiveTest, MessageCodes code)
	{
		...
		ici.usage = VK_IMAGE_USAGE_SAMPLED_BIT |
		            VK_IMAGE_USAGE_TRANSFER_DST_BIT; //usage flags don't indicate render target usage
		if (!positiveTest)
		{
			ici.usage |=
			    VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT; //add render target usage flag if we want a negative result
		}
		...
		setImageLayout(device, queue, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, image, 0, 1,
	...
	//test with uncompressed texture format
	bool testSuboptimalFormat(bool positiveTest)
	{
		auto tex = make_shared<Texture>(device);
		tex->initRenderTarget2D(128, 128,
		                        positiveTest ? VK_FORMAT_B10G11R11_UFLOAT_PACK32 : VK_FORMAT_A2B10G10R10_UNORM_PACK32);

		uint32_t count = getCount(MESSAGE_CODE_SUBOPTIMAL_TEXTURE_FORMAT);
		resetCounts();
	...
	bool testLinearTexture(bool positiveTest)
	{
		VkImageCreateInfo ici = { VK_STRUCTURE_TYPE_IMAGE_CREATE_INFO };
		ici.arrayLayers = 1;
		ici.extent.width = 128;
		...
		ici.mipLevels = 1;
		ici.samples = VK_SAMPLE_COUNT_1_BIT;
		ici.tiling = positiveTest ? VK_IMAGE_TILING_LINEAR : VK_IMAGE_TILING_OPTIMAL;
		ici.usage = VK_IMAGE_USAGE_SAMPLED_BIT |
		...
		uint32_t count = getCount(MESSAGE_CODE_TEXTURE_LINEAR_TILING);
		resetCounts();
	...
	{
		if (!testUncompressedFormat(false, MESSAGE_CODE_UNCOMPRESSED_TEXTURE_USED))
			return false;

		if (!testUncompressedFormat(true, MESSAGE_CODE_UNCOMPRESSED_TEXTURE_USED))
			return false;

		if (!testUncompressedFormat(false, MESSAGE_CODE_NON_MIPMAPPED_TEXTURE_USED))
			return false;

		if (!testUncompressedFormat(true, MESSAGE_CODE_NON_MIPMAPPED_TEXTURE_USED))
			return false;

		if (!testSuboptimalFormat(false))
			return false;

		if (!testSuboptimalFormat(true))
			return false;

		if (!testLinearTexture(false))
			return false;

		if (!testLinearTexture(true))
			return false;
