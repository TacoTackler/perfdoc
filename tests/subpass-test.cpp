...
class SubpassTest : public VulkanTestHelper
{
public:
	bool testOps(bool positiveTest)
	{
		...
		attachments[0].stencilLoadOp = positiveTest ? VK_ATTACHMENT_LOAD_OP_LOAD : VK_ATTACHMENT_LOAD_OP_CLEAR;
		...
		uint32_t count = getCount(MESSAGE_CODE_INEFFICIENT_DEPTH_STENCIL_OPS);
		resetCounts();
	...
	bool testDependency(bool positiveTest)
	{
		VkAttachmentDescription attachments[1] = {};
		...
		subpass[0].pColorAttachments = &attachment;
		subpass[0].pipelineBindPoint = VK_PIPELINE_BIND_POINT_GRAPHICS;

		subpass[1].colorAttachmentCount = 1;
		subpass[1].pColorAttachments = &attachment;
		subpass[1].pipelineBindPoint = VK_PIPELINE_BIND_POINT_GRAPHICS;

		VkSubpassDependency deps = {};
		deps.srcSubpass = 0;
		deps.dstSubpass = 1;
		deps.srcStageMask = VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT | VK_PIPELINE_STAGE_LATE_FRAGMENT_TESTS_BIT;
		deps.dstStageMask = VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT | VK_PIPELINE_STAGE_EARLY_FRAGMENT_TESTS_BIT;
		deps.srcAccessMask = VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT;
		deps.dstAccessMask = VK_ACCESS_INPUT_ATTACHMENT_READ_BIT;
		deps.dependencyFlags = positiveTest ? 0 : VK_DEPENDENCY_BY_REGION_BIT;
		...
		uint32_t count = getCount(MESSAGE_CODE_SUBOPTIMAL_SUBPASS_DEPENDENCY_FLAG);
		resetCounts();
	...
	bool testStencilSelfDependency(bool positiveTest)
	{
		...
		const VkAttachmentReference attachment = { 0, VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL };
		const VkAttachmentReference inputAttachment = { 1, VK_IMAGE_LAYOUT_DEPTH_STENCIL_READ_ONLY_OPTIMAL };

		VkSubpassDescription subpass = {};
		subpass.colorAttachmentCount = 0;
		subpass.pDepthStencilAttachment = &attachment;
		subpass.pipelineBindPoint = VK_PIPELINE_BIND_POINT_GRAPHICS;
		subpass.inputAttachmentCount = positiveTest ? 1 : 0;
		subpass.pInputAttachments = &inputAttachment;

		VkSubpassDependency deps = {};
		deps.srcSubpass = 0;
		deps.dstSubpass = 0;
		deps.srcStageMask = VK_PIPELINE_STAGE_ALL_GRAPHICS_BIT;
		deps.dstStageMask = VK_PIPELINE_STAGE_ALL_GRAPHICS_BIT;
		deps.srcAccessMask = VK_ACCESS_DEPTH_STENCIL_ATTACHMENT_WRITE_BIT;
		deps.dstAccessMask = VK_ACCESS_INPUT_ATTACHMENT_READ_BIT;
		deps.dependencyFlags = VK_DEPENDENCY_BY_REGION_BIT;

		VkRenderPassCreateInfo info = { VK_STRUCTURE_TYPE_RENDER_PASS_CREATE_INFO };
		info.attachmentCount = positiveTest ? 2 : 1;
		info.subpassCount = 1;
		info.pSubpasses = &subpass;
		info.pAttachments = &attachments[0];
		info.dependencyCount = positiveTest ? 1 : 0;
		info.pDependencies = &deps;
		...
		uint32_t count = getCount(MESSAGE_CODE_SUBPASS_STENCIL_SELF_DEPENDENCY);
		resetCounts();
	...
	bool testPotentialSubpass(bool positiveTest)
	{
		VkAttachmentDescription attachments[1] = {};
		attachments[0].initialLayout = VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL;
		attachments[0].finalLayout = VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL;
...
#include "quad.frag.inc"
		...
		for (int c = 0; c < (positiveTest ? 2 : 1); ++c)
		{
		...
		uint32_t count = getCount(MESSAGE_CODE_POTENTIAL_SUBPASS);
		resetCounts();
	...
	bool runTest() override
	{
		...
		if (!testStencilSelfDependency(true))
			return false;

		if (!testStencilSelfDependency(false))
			return false;

		if (!testDependency(false))
			return false;

		if (!testDependency(true))
			return false;

		if (!testPotentialSubpass(true))
			return false;

		if (!testPotentialSubpass(false))
			return false;
